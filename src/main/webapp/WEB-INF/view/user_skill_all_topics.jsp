...
<body class="w3-light-grey w3-content" style="max-width: 1600px">
	...
	<div class="w3-main" style="margin-left: 300px">
		...
		<div style="float: left; width: 20%;">
			<br> <br> <br>
			<form action="topicDetails">
				<input type="hidden" name="skillId" value="${currentTopic.skillId }">
				<c:forEach var="t" items="${topicList}">
					<%
					Topic topic=(Topic)pageContext.getAttribute("t");
					String cssClass="";
					if(topic.getTopicId()==((Topic)request.getAttribute("currentTopic")).getTopicId()){
						cssClass="w3-button w3-block w3-black ";
					}else{
						cssClass="w3-button w3-block w3-grey ";
					}
					
					%>
					<button type="submit" style="margin-bottom:5px"
						class="<%=cssClass %>"
						value="${t.topicId }" name="topicId">${t.topic }</button>
				</c:forEach>
			</form>
		</div>
		...
		<div style="float: right; width: 80%;">
			...
			<div class="scp-quizzes-data" style="padding-left: 4%;">


				<h3>${currentTopic.topic }</h3>
				<p>

					<br> ${currentTopic.content }


				</p>
				<br>
				<iframe width="560" height="315"
					src="${currentTopic.url }" frameborder="0"
					allowfullscreen></iframe>
				<br>
				<form action="topicDetails">
					<input type="hidden" name="skillId"
						value="${currentTopic.skillId }">
					<% if((long)request.getAttribute("preTopic")!=0){%>
					<button type="submit" name="topicId" value="${preTopic }"
						class="w3-button w3-white w3-padding-small">
						<b>previous</b>
					</button>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<% }%>
					<% if((long)request.getAttribute("nextTopic")!=0){%><button type="submit" name="topicId" value="${nextTopic }"
						/class="w3-button w3-white w3-padding-small">
						<b>next</b>
					</button>
					<%} %>
